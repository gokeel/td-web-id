...
class Payroll extends MY_Controller {
	...
	function calculation_start(){
		...
		$data = array(
			...
			'title' => 'Calculation period '.$start_date.' '.$month_prev_str.' - '.$end_date.' '.$month_now_str.' '.$year_now
			);
		
		$data['data_monitoring'] = $this->Payroll_m->get_not_final_course_monitoring($year_now.'-'.$month_now.'-'.$start_date);
		$data['date_param'] = $year_now.'-'.$month_now.'-'.$start_date;
	...
	function monitoring_validation(){
		...
		$data['absence'] = $this->Payroll_m->get_not_final_course_monitoring($year_now.'-'.$month_now.'-'.$start_date, $this->input->get('id', TRUE));
	...
	function finalization_start(){
		...
		$data = array(
			...
			'title' => 'Finalisasi Gaji Guru Periode '.$start_date.' '.$month_prev_str.' - '.$end_date.' '.$month_now_str.' '.$year_now,
			'period' => get_month_string($month_now).' '.$year_now,
			'period_in_numeric_format' => $year_now.$month_now,
			'filter_param' => $year_now.'-'.$month_now.'-'.$start_date
			);
		
		$data['data_final'] = $this->Payroll_m->get_monitoring_validation_not_final($year_now.'-'.$month_now.'-'.$start_date);
		// $this->export_query($this->db->last_query());
	...
	function commit($date, $period){
		...
		foreach($fix_data_per_teacher->result() as $teacher){
			...
			// get salary per hour
			$salary_per_hour = $teacher->salary_per_hour;

			$total_salary = ($percentage_fee / 100) * $total_hours * $salary_per_hour;
